...
export default function BarberServicesManagementPage() {
  ...
  useEffect(() => {
    ...
    // 2. Load services for this tenant
  ...
  const handleOpenEditModal = (svc: Service) => {
    ...
    setFormDuration(svc.duration.toString());
  ...
  const handleSaveService = (e: React.FormEvent) => {
    ...
    // Load full globally scoped services list
    ...
    if (editingService) {
      ...
      setSuccessToast(`Treatment "${formName}" has been modified successfully inside local ledger.`);
    } else {
      // ADD WORKFLOW
      const randomId = 'svc-' + Math.floor(100 + Math.random() * 900);
      ...
      setSuccessToast(`New Treatment "${formName}" has been provisionsed successfully.`);
    }
  ...
  const handleToggleStatus = (svcId: string) => {
    ...
    const updatedMaster = masterServices.map(item => {
      if (item.id === svcId) {
        const nextStatusValue = item.status === 'Active' ? 'Inactive' as const : 'Active' as const;
        setSuccessToast(`Treatment status for ${item.name} set to ${nextStatusValue}.`);
        return { ...item, status: nextStatusValue };
      }
      return item;
    });
  ...
  const handleDeleteService = (svcId: string) => {
    ...
    setSuccessToast(`Treatment has been deleted.`);
    setTimeout(() => setSuccessToast(null), 3000);
  };
  ...
  return (
    <div className="flex h-screen bg-[#fafaf9] overflow-hidden font-sans" id="barber-services-page">
      <Sidebar />
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        {successToast && (
          <div className="fixed top-5 right-5 z-50 max-w-sm bg-[#1a1a1a] text-white border-l-4 border-[#d4a574] p-4 rounded shadow-xl animate-fade-in" id="toast-success-ledger">
        ...
        <main className="p-6 md:p-8 space-y-8 max-w-6xl w-full mx-auto" id="barber-services-content">
          
          {/* Header segment */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-neutral-200 pb-6" id="services-header">
            <div>
              ...
              <p className="text-xs text-neutral-500 mt-1">Publish and adjust treatments, prices, times, and scheduling active states.</p>
            </div>
            ...
            <button 
              ...
              id="add-service-btn"
            >
              <Plus className="h-4.5 w-4.5" /> Add Treatment
            </button>
          </div>

          {/* Table filters */}
          <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-4 flex flex-col sm:flex-row items-center gap-4 justify-between" id="services-filters">
            ...
            <p className="text-[10px] text-neutral-400 font-mono">
              Total Catalog count: <span className="text-[#1a1a1a] font-bold">{filteredServices.length} Published</span>
            </p>
          </div>

          {/* MAIN DATATABLE LIST */}
          <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm" id="services-table-wrapper">
            {filteredServices.length === 0 ? (
              <div className="text-center py-16 space-y-3">
                <Scissors className="h-10 w-10 text-neutral-300 mx-auto" />
                <p className="text-xs text-neutral-400">No published treatment services match filter query.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-[#fafaf9] text-neutral-400 border-b border-neutral-250 text-[10px] font-bold uppercase tracking-wider">
                      <th className="p-4">Service Name / Option</th>
                  ...
                  <tbody className="divide-y divide-neutral-100 font-medium">
                    {filteredServices.map((svc) => (
                      <tr key={svc.id} className="hover:bg-[#fafaf9]/40" id={`service-row-${svc.id}`}>
                        ...
                        <td className="p-4 text-neutral-800 font-sans">
                          <span className="inline-flex items-center gap-1 bg-neutral-100 px-2 py-1 rounded text-[10px]">
                            <Clock className="h-3 w-3 text-[#8b7355]" /> {svc.duration} Minutes
                          </span>
                        </td>
                        ...
                        <td className="p-4">
                          <button 
                            ...
                            className={`px-2.5 py-1 text-[9px] font-black uppercase tracking-wider rounded-full border transition duration-200 ${svc.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' : 'bg-neutral-100 text-neutral-500 border-neutral-200 hover:bg-neutral-200'}`}
                            id={`status-toggle-${svc.id}`}
      ...
      {deleteConfirmationId && (
        <div className="fixed inset-0 bg-[#1a1a1a]/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm" id="delete-service-modal">
          <div className="bg-white border border-[#8b7355]/30 rounded-xl max-w-sm w-full overflow-hidden shadow-2xl animate-fade-in text-[#1a1a1a]">
            <div className="p-6 text-center space-y-4">
              ...
              <div className="space-y-1.5">
                <h3 className="font-serif font-bold text-lg text-neutral-900">Purge Treatment Service?</h3>
                <p className="text-xs text-neutral-500 leading-relaxed font-sans">
                  This action is irreversible and will remove this grooming treatment from the customer scheduling grid. Continuing?
                </p>
              </div>
            </div>
            ...
            <div className="bg-neutral-50 px-6 py-4 flex items-center justify-end gap-3 font-bold text-xs uppercase border-t border-neutral-100">
              ...
              <button 
                onClick={() => handleDeleteService(deleteConfirmationId)}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-[#1a1a1a]/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm" id="catalog-form-modal">
          <div className="bg-white border border-[#8b7355]/30 rounded-xl max-w-md w-full overflow-hidden shadow-2xl animate-fade-in text-[#1a1a1a]">
            <div className="bg-[#1a1a1a] text-white p-6 border-b border-[#8b7355]/20 flex items-center justify-between">
              <div>
                <h3 className="font-serif font-bold text-lg text-white">
                  {editingService ? 'Modify Treatment' : 'Publish New Treatment'}
                </h3>
                <p className="text-[10px] text-zinc-400 mt-1">Settings immediately affect guest booking scheduling options.</p>
              </div>
              <button 
            ...
            <form onSubmit={handleSaveService} className="p-6 space-y-4">
              <div>
                <label className="text-[10px] font-bold uppercase text-neutral-400 block mb-1">Service Option Name <span className="text-amber-600">*</span></label>
                <input 
                  type="text" 
                  required
                  placeholder="e.g. Skin Fade & Bay Rum Shave"
                  value={formName}
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-neutral-400 block mb-1">Catalog Description <span className="text-amber-600">*</span></label>
                <textarea 
                  rows={3}
                  required
                  placeholder="Articulate service benefits, straight razor lining details, massage sequence, or essential oil styling finishes..."
                  value={formDescription}
                  onChange={(e) => setFormDescription(e.target.value)}
                  className="w-full border border-neutral-300 rounded p-2 text-xs focus:outline-none focus:ring-1 focus:ring-[#d4a574] font-medium font-sans"
                  id="form-svc-desc"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold uppercase text-neutral-400 block mb-1">Session Duration (Mins)</label>
                  <select
                    value={formDuration}
                    onChange={(e) => setFormDuration(e.target.value)}
                    className="w-full border border-neutral-300 rounded p-2 text-xs focus:outline-none focus:ring-1 focus:ring-[#d4a574] font-medium"
                    id="form-svc-duration"
                  >
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase text-neutral-400 block mb-1">Billing Price ($ USD) <span className="text-amber-600">*</span></label>
                  <input 
                    type="number" 
                    required
                    min="1"
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-neutral-400 block mb-1">Initial Visibility Status</label>
